...
type BootOptions struct {
	...
	TiDBSystem     instance.Config        `yaml:"tidb_system"`
...
`, options.Version, version.String())
	...
	rootCmd.Flags().IntVar(&options.TiDBSystem.Num, "db_system", 0, "TiDB system instance number")
	...
	rootCmd.Flags().StringVar(&options.TiDBSystem.Host, "db_system.host", "", "Playground TiDB system host. If not provided, TiDB will still use `host` flag as its host")
	rootCmd.Flags().IntVar(&options.TiDBSystem.Port, "db_system.port", 0, "Playground TiDB system port. If not provided, TiDB will use 4000 as its port. Or 6000 if TiProxy is enabled.")
	...
	rootCmd.Flags().StringVar(&options.TiDBSystem.ConfigPath, "db_system.config", "", "TiDB system instance configuration file")
	...
	rootCmd.Flags().StringVar(&options.TiDBSystem.BinPath, "db_system.binpath", "", "TiDB system instance binary path")
...
func populateDefaultOpt(flagSet *pflag.FlagSet) error {
	...
	case "tidb-nextgen":
		defaultInt(&options.TiDB.Num, "db", 1)
		defaultInt(&options.TiKV.Num, "kv", 1)
		defaultInt(&options.TiKVWorker.Num, "kv", 1)
	case "tikv-slim":
		defaultInt(&options.TiKV.Num, "kv", 1)
